...
pub const SRV_KEY_RETR_SERVICE_CONTRACT_REGISTRY_NAME: &'static str =
    "secretstore_service_srv_retr";
/// Name of the document key store SecretStore contract in the registry.
pub const DOC_KEY_STORE_SERVICE_CONTRACT_REGISTRY_NAME: &'static str =
    "secretstore_service_doc_store";
/// Name of the document key retrieval SecretStore contract in the registry.
pub const DOC_KEY_SRETR_SERVICE_CONTRACT_REGISTRY_NAME: &'static str =
    "secretstore_service_doc_sretr";
...
pub trait ServiceContract: Send + Sync {
    ...
    fn publish_server_key_generation_error(
        &self,
        origin: &Address,
        server_key_id: &ServerKeyId,
    ) -> Result<(), String>;
    ...
    fn publish_stored_document_key(
        &self,
        origin: &Address,
        server_key_id: &ServerKeyId,
    ) -> Result<(), String>;
    ...
    /// Publish document key store error.
    fn publish_document_key_retrieval_error(
        &self,
        origin: &Address,
        server_key_id: &ServerKeyId,
        requester: &Address,
...
impl OnChainServiceContract {
    ...
    fn send_contract_transaction<C, P>(
        &self,
        tx_name: &str,
        origin: &Address,
        server_key_id: &ServerKeyId,
        is_response_required: C,
        prepare_tx: P,
    ) -> Result<(), String>
    where
        C: FnOnce(&Client, &Address, &ServerKeyId, &Address) -> bool,
        P: FnOnce(&Client, &Address) -> Result<Bytes, String>,
    {
        ...
        // send transaction
        self.client
            .transact_contract(origin.clone(), transaction_data)
            .map_err(|e| format!("{}", e))?;
    ...
    fn create_pending_requests_iterator<
        ...
        R: 'static
            + Fn(
                ...
                &BlockId,
                U256,
            ) -> Result<(bool, ServiceTask), String>,
    >(
        &self,
        client: Arc<Client>,
        ...
        get_count: C,
        read_item: R,
    ) -> Box<dyn Iterator<Item = (bool, ServiceTask)>> {
        get_count(&*client, contract_address, block)
            .map(|count| {
                ...
                Box::new(PendingRequestsIterator {
                    read_request: move |index| {
                        read_item(&*self_key_pair, &*client, &contract_address, &block, index)
						.map_err(|error| {
    ...
    fn update_contract_address(&self) -> bool {
        let contract_address = self
            .client
            .read_contract_address(self.name.clone(), &self.address_source);
...
impl ServiceContract for OnChainServiceContract {
    ...
    fn read_logs(&self) -> Box<dyn Iterator<Item = ServiceTask>> {
        ...
        let (address, first_block, last_block) = {
            ...
            let confirmed_block =
                match get_confirmed_block_hash(&*client, REQUEST_CONFIRMATIONS_REQUIRED) {
                    Some(confirmed_block) => confirmed_block,
                    None => return Box::new(::std::iter::empty()), // no block with enough confirmations
                };
            let first_block = match data
                .last_log_block
                .take()
                .and_then(|b| client.tree_route(&b, &confirmed_block))
            {
        ...
        Box::new(
            request_logs
                ...
                .filter_map(|log| {
                    let raw_log: RawLog = (
                        log.entry.topics.into_iter().map(|t| t.0.into()).collect(),
                        log.entry.data,
                    )
                        .into();
    ...
    fn read_pending_requests(&self) -> Box<dyn Iterator<Item = (bool, ServiceTask)>> {
        ...
        match data.contract_address {
            ...
            Some(contract_address) => {
                get_confirmed_block_hash(&*client, REQUEST_CONFIRMATIONS_REQUIRED + 1)
                    .map(|b| {
                        let block = BlockId::Hash(b);
                        let iter = match self.mask.server_key_generation_requests {
                            true => Box::new(self.create_pending_requests_iterator(
                                client.clone(),
                                &contract_address,
                                &block,
                        ...
                        let iter = match self.mask.server_key_retrieval_requests {
                            true => Box::new(iter.chain(self.create_pending_requests_iterator(
                                client.clone(),
                                &contract_address,
                                &block,
    ...
    fn publish_generated_server_key(
        ...
        self.send_contract_transaction(
            "publish_generated_server_key",
            origin,
            server_key_id,
            ServerKeyGenerationService::is_response_required,
            |_, _| {
                Ok(ServerKeyGenerationService::prepare_pubish_tx_data(
                    server_key_id,
                    &server_key,
                ))
            },
    ...
    fn publish_server_key_generation_error(
        ...
        self.send_contract_transaction(
            "publish_server_key_generation_error",
            origin,
            server_key_id,
            ServerKeyGenerationService::is_response_required,
            |_, _| {
                Ok(ServerKeyGenerationService::prepare_error_tx_data(
                    server_key_id,
                ))
            },
        )
    ...
    fn publish_retrieved_server_key(
        ...
        self.send_contract_transaction(
            "publish_retrieved_server_key",
            origin,
            server_key_id,
            ServerKeyRetrievalService::is_response_required,
            |_, _| {
                Ok(ServerKeyRetrievalService::prepare_pubish_tx_data(
                    server_key_id,
                    server_key,
                    threshold,
                ))
    ...
    fn publish_server_key_retrieval_error(
        ...
        self.send_contract_transaction(
            "publish_server_key_retrieval_error",
            origin,
            server_key_id,
            ServerKeyRetrievalService::is_response_required,
            |_, _| {
                Ok(ServerKeyRetrievalService::prepare_error_tx_data(
                    server_key_id,
                ))
            },
        )
    ...
    fn publish_stored_document_key(
        ...
        self.send_contract_transaction(
            "publish_stored_document_key",
            origin,
            server_key_id,
            DocumentKeyStoreService::is_response_required,
            |_, _| {
                Ok(DocumentKeyStoreService::prepare_pubish_tx_data(
                    server_key_id,
                ))
            },
        )
    ...
    fn publish_document_key_store_error(
        ...
        self.send_contract_transaction(
            "publish_document_key_store_error",
            origin,
            server_key_id,
            DocumentKeyStoreService::is_response_required,
            |_, _| {
                Ok(DocumentKeyStoreService::prepare_error_tx_data(
                    server_key_id,
                ))
            },
        )
    ...
    fn publish_retrieved_document_key_common(
        ...
        self.send_contract_transaction(
            ...
            |client, contract_address, server_key_id, key_server| {
                DocumentKeyShadowRetrievalService::is_response_required(
                    client,
                    contract_address,
                    server_key_id,
                    requester,
            ...
            |_, _| {
                Ok(
                    DocumentKeyShadowRetrievalService::prepare_pubish_common_tx_data(
                        server_key_id,
                        requester,
                        common_point,
                        threshold,
    ...
    fn publish_retrieved_document_key_personal(
        ...
        self.send_contract_transaction(
            ...
            |_, _, _, _| true,
            move |client, address| {
                DocumentKeyShadowRetrievalService::prepare_pubish_personal_tx_data(
                    client,
                    address,
                    server_key_id,
                    requester,
    ...
    fn publish_document_key_retrieval_error(
        ...
        self.send_contract_transaction(
            "publish_document_key_retrieval_error",
            origin,
            server_key_id,
            |client, contract_address, server_key_id, key_server| {
                DocumentKeyShadowRetrievalService::is_response_required(
            ...
            |_, _| {
                Ok(DocumentKeyShadowRetrievalService::prepare_error_tx_data(
                    server_key_id,
                    requester,
                ))
            },
...
impl<F> Iterator for PendingRequestsIterator<F>
where
    F: Fn(U256) -> Option<(bool, ServiceTask)>,
{
...
impl ServerKeyGenerationService {
    ...
    pub fn parse_log(origin: &Address, raw_log: RawLog) -> Result<ServiceTask, String> {
        match service::events::server_key_generation_requested::parse_log(raw_log) {
            Ok(l) => Ok(ServiceTask::GenerateServerKey(
                origin.clone(),
                l.server_key_id,
                l.author,
                parse_threshold(l.threshold)?,
            )),
            Err(e) => Err(format!("{}", e)),
        }
    }
    ...
    pub fn is_response_required(
        ...
        let (encoded, decoder) =
            service::functions::is_server_key_generation_response_required::call(
                *server_key_id,
                *key_server,
            );
    ...
    fn read_pending_requests_count(
        client: &Client,
        contract_address: &Address,
        block: &BlockId,
    ) -> Result<U256, String> {
        ...
        decoder
            .decode(&client.call_contract(*block, *contract_address, encoded)?)
    ...
    fn read_pending_request(
        ...
        let (server_key_id, author, threshold) = decoder
            .decode(&client.call_contract(*block, *contract_address, encoded)?)
            .map_err(|e| e.to_string())?;
...
impl ServerKeyRetrievalService {
    ...
    pub fn is_response_required(
        ...
        // we're checking confirmation in Latest block, because we're interested in latest contract state here
        let (encoded, decoder) =
            service::functions::is_server_key_retrieval_response_required::call(
                *server_key_id,
                *key_server,
            );
        match client.call_contract(BlockId::Latest, *contract_address, encoded) {
    ...
    fn read_pending_request(
        ...
        let task = ServiceTask::RetrieveServerKey(*contract_address, server_key_id);
...
impl DocumentKeyStoreService {
    ...
    fn read_pending_request(
        ...
        let (server_key_id, author, common_point, encrypted_point) = decoder
            .decode(&client.call_contract(*block, *contract_address, encoded)?)
            .map_err(|e| e.to_string())?;
...
impl DocumentKeyShadowRetrievalService {
    ...
    pub fn parse_common_request_log(
        origin: &Address,
        raw_log: RawLog,
    ) -> Result<ServiceTask, String> {
        match service::events::document_key_common_retrieval_requested::parse_log(raw_log) {
            Ok(l) => Ok(ServiceTask::RetrieveShadowDocumentKeyCommon(
                origin.clone(),
                l.server_key_id,
                l.requester,
            )),
    ...
    pub fn parse_personal_request_log(
        origin: &Address,
        raw_log: RawLog,
    ) -> Result<ServiceTask, String> {
        match service::events::document_key_personal_retrieval_requested::parse_log(raw_log) {
            Ok(l) => Ok(ServiceTask::RetrieveShadowDocumentKeyPersonal(
                origin.clone(),
                l.server_key_id,
                (*l.requester_public).into(),
            )),
            Err(e) => Err(e.to_string()),
        }
    }
    ...
    pub fn is_response_required(
        ...
        // we're checking confirmation in Latest block, because we're interested in latest contract state here
        let (encoded, decoder) =
            service::functions::is_document_key_shadow_retrieval_response_required::call(
                *server_key_id,
                *requester,
                *key_server,
            );
    ...
    pub fn prepare_pubish_personal_tx_data(
        client: &Client,
        contract_address: &Address,
        server_key_id: &ServerKeyId,
        requester: &Address,
        ...
        for participant in participants {
            let participant_index =
                Self::map_key_server_address(client, contract_address, participant.clone())
                    .map_err(|e| {
                        format!("Error searching for {} participant: {}", participant, e)
                    })?;
            participants_mask = participants_mask | (U256::one() << participant_index);
        }
        Ok(
            service::functions::document_key_personal_retrieved::encode_input(
                *server_key_id,
                *requester,
                participants_mask,
    ...
    fn read_pending_requests_count(
        ...
        let (encoded, decoder) =
            service::functions::document_key_shadow_retrieval_requests_count::call();
        decoder
            .decode(&client.call_contract(*block, *contract_address, encoded)?)
            .map_err(|e| e.to_string())
    }
    ...
    fn read_pending_request(
        ...
        let (encoded, decoder) =
            service::functions::get_document_key_shadow_retrieval_request::call(index);
        let (server_key_id, requester, is_common_retrieval_completed) = decoder
            .decode(&client.call_contract(*block, *contract_address, encoded)?)
            .map_err(|e| e.to_string())?;
    ...
    fn map_key_server_address(
        ...
        let index = decoder
            .decode(&client.call_contract(BlockId::Latest, *contract_address, encoded)?)
            .map_err(|e| e.to_string())?;
...
fn parse_threshold(threshold: U256) -> Result<usize, String> {
    ...
    if threshold != threshold_num.into() || threshold_num >= ::std::u8::MAX as u64 {
        return Err(format!(
            "invalid threshold to use in service contract: {}",
            threshold
        ));
    }
...
pub mod tests {
    ...
    pub struct DummyServiceContract {
        ...
        pub common_shadow_retrieved_document_keys:
            Mutex<Vec<(ServerKeyId, Address, Public, usize)>>,
        pub personal_shadow_retrieved_document_keys:
            Mutex<Vec<(ServerKeyId, Address, Vec<Address>, Public, Bytes)>>,
    ...
    impl ServiceContract for DummyServiceContract {
        ...
        fn publish_generated_server_key(
            ...
            self.generated_server_keys
                .lock()
                .push((server_key_id.clone(), server_key.clone()));
            Ok(())
        }
        ...
        fn publish_server_key_generation_error(
            ...
            self.server_keys_generation_failures
                .lock()
                .push(server_key_id.clone());
            Ok(())
        }
        ...
        fn publish_retrieved_server_key(
            ...
            self.retrieved_server_keys.lock().push((
                server_key_id.clone(),
                server_key.clone(),
                threshold,
            ));
        ...
        fn publish_server_key_retrieval_error(
            ...
            self.server_keys_retrieval_failures
                .lock()
                .push(server_key_id.clone());
            Ok(())
        }
        ...
        fn publish_document_key_store_error(
            ...
            self.document_keys_store_failures
                .lock()
                .push(server_key_id.clone());
            Ok(())
        }
        ...
        fn publish_retrieved_document_key_common(
            ...
            self.common_shadow_retrieved_document_keys.lock().push((
                server_key_id.clone(),
                requester.clone(),
                common_point.clone(),
                threshold,
        ...
        fn publish_retrieved_document_key_personal(
            ...
            self.personal_shadow_retrieved_document_keys.lock().push((
                server_key_id.clone(),
                requester.clone(),
        ...
        fn publish_document_key_retrieval_error(
            ...
            self.document_keys_shadow_retrieval_failures
                .lock()
                .push((server_key_id.clone(), requester.clone()));
            Ok(())
        }
    }
}
