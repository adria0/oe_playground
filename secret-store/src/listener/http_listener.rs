...
use hyper::{
    ...
    server::conn::Http,
    service::Service,
    Body, Method as HttpMethod, Request as HttpRequest, Response as HttpResponse,
...
fn return_error(err: Error) -> HttpResponse<Body> {
    let status = match err {
        ...
        Error::ServerKeyIsNotFound | Error::DocumentKeyIsNotFound => HttpStatusCode::NOT_FOUND,
        Error::InsufficientRequesterData(_)
        | Error::Hyper(_)
        | Error::Serde(_)
        | Error::DocumentKeyAlreadyStored
        | Error::ServerKeyAlreadyGenerated => HttpStatusCode::BAD_REQUEST,
...
fn parse_request(method: &HttpMethod, uri_path: &str, body: &[u8]) -> Request {
    ...
    let is_known_prefix = &path[0] == "shadow"
        || &path[0] == "schnorr"
        || &path[0] == "ecdsa"
        || &path[0] == "server";
    let (prefix, args_offset) = if is_known_prefix {
        (&*path[0], 1)
    } else {
        ("", 0)
    };
    ...
    match (
        ...
        ("server", 2, &HttpMethod::GET, _, _, _, _) => Request::GetServerKey(document, signature),
...
mod tests {
    ...
    fn http_listener_successfully_drops() {
        ...
        let listener =
            KeyServerHttpListener::start(address, Arc::downgrade(&key_server), runtime.executor())
                .unwrap();
        drop(listener);
    }
