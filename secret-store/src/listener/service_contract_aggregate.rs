...
use ethkey::Public;
use listener::{service_contract::ServiceContract, service_contract_listener::ServiceTask};
use std::sync::Arc;
use ServerKeyId;
...
impl ServiceContract for OnChainServiceContractAggregate {
    ...
    fn read_logs(&self) -> Box<dyn Iterator<Item = ServiceTask>> {
        self.contracts.iter().fold(
            Box::new(::std::iter::empty()) as Box<dyn Iterator<Item = ServiceTask>>,
            |i, c| Box::new(i.chain(c.read_logs())),
        )
    }

    fn read_pending_requests(&self) -> Box<dyn Iterator<Item = (bool, ServiceTask)>> {
        self.contracts.iter().fold(
            Box::new(::std::iter::empty()) as Box<dyn Iterator<Item = (bool, ServiceTask)>>,
            |i, c| Box::new(i.chain(c.read_pending_requests())),
        )
    }
    ...
    fn publish_retrieved_document_key_common(
        ...
        self.contracts[0].publish_retrieved_document_key_common(
            origin,
            server_key_id,
            requester,
            common_point,
    ...
    fn publish_retrieved_document_key_personal(
        ...
        self.contracts[0].publish_retrieved_document_key_personal(
            origin,
            server_key_id,
            requester,
            participants,
