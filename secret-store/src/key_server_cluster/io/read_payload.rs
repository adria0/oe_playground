...
use key_server_cluster::{
    io::message::{decrypt_message, deserialize_message, MessageHeader},
    message::Message,
    Error,
};
use std::io;
...
pub fn read_payload<A>(a: A, header: MessageHeader) -> ReadPayload<A>
where
    A: AsyncRead,
{
    ReadPayload {
...
pub fn read_encrypted_payload<A>(a: A, header: MessageHeader, key: KeyPair) -> ReadPayload<A>
where
    A: AsyncRead,
{
    ReadPayload {
...
impl<A> Future for ReadPayload<A>
where
    A: AsyncRead,
{
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        ...
        let payload = if let Some(key) = self.key.take() {
            decrypt_message(&key, data).and_then(|data| deserialize_message(&self.header, data))
        } else {
