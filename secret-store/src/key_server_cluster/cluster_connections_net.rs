...
use key_server_cluster::{
    cluster_connections::{Connection, ConnectionManager, ConnectionProvider},
    cluster_message_processor::MessageProcessor,
    connection_trigger::{ConnectionTrigger, Maintain},
    io::{
        read_encrypted_message, write_encrypted_message, DeadlineStatus, ReadMessage,
        SharedTcpStream, WriteMessage,
    },
    message::{self, ClusterMessage, Message},
    net::{
        accept_connection as io_accept_connection, connect as io_connect,
        Connection as IoConnection,
    },
    ClusterConfiguration, Error, NodeId, NodeKeyPair,
};
...
use tokio::{
    ...
    timer::{timeout::Error as TimeoutError, Interval},
};
...
impl NetConnectionsManager {
    ...
    pub fn new(
        ...
        let listen_address =
            make_socket_address(&net_config.listen_address.0, net_config.listen_address.1)?;
...
impl Connection for NetConnection {
    ...
    fn send_message(&self, message: Message) {
        execute(
            &self.executor,
            self.send_message_future(message).then(|_| Ok(())),
        );
    }
}
...
fn net_listen(listen_address: &SocketAddr, data: Arc<NetConnectionsData>) -> Result<(), Error> {
    execute(
        &data.executor,
        net_listen_future(listen_address, data.clone())?,
    );
    Ok(())
}
...
fn net_accept_connection(data: Arc<NetConnectionsData>, stream: TcpStream) {
    execute(
        &data.executor,
        net_accept_connection_future(data.clone(), stream),
    );
}
...
fn net_accept_connection_future(
    ...
    Box::new(
        io_accept_connection(stream, data.self_key_pair.clone())
...
fn net_connect(data: Arc<NetConnectionsData>, remote: SocketAddr) {
    execute(&data.executor, net_connect_future(data.clone(), remote));
}

/// Connect to remote node future.
fn net_connect_future(data: Arc<NetConnectionsData>, remote: SocketAddr) -> BoxedEmptyFuture {
    let disconnected_nodes = data.container.disconnected_nodes();
    Box::new(
        io_connect(&remote, data.self_key_pair.clone(), disconnected_nodes)
...
fn net_process_connection_result(
    ...
    match result {
        Ok(DeadlineStatus::Meet(Ok(connection))) => {
            let connection = Arc::new(NetConnection::new(
                data.executor.clone(),
                outbound_addr.is_none(),
                connection,
            ));
            if data.insert(connection.clone()) {
                let maintain_action = data
                    .trigger
                    .lock()
...
fn net_schedule_maintain(data: Arc<NetConnectionsData>) {
    ...
    execute(
        &data.executor,
        Interval::new_interval(Duration::new(MAINTAIN_INTERVAL, 0))
...
fn execute<F: Future<Item = (), Error = ()> + Send + 'static>(executor: &Executor, f: F) {
    if let Err(err) = future::Executor::execute(executor, Box::new(f)) {
        error!(
            "Secret store runtime unable to spawn task. Runtime is shutting down. ({:?})",
            err
        );
    }
}
...
fn maintain_connection_trigger(data: Arc<NetConnectionsData>, maintain_action: Option<Maintain>) {
        ...
        || maintain_action == Some(Maintain::Session)
    {
        ...
        if let Some(session_params) = session_params {
            let session = data
                .message_processor
                .start_servers_set_change_session(session_params);
            match session {
                Ok(_) => trace!(target: "secretstore_net", "{}: started auto-migrate session",
					data.self_key_pair.public()),
                Err(err) => {
                    trace!(target: "secretstore_net", "{}: failed to start auto-migrate session with: {}",
