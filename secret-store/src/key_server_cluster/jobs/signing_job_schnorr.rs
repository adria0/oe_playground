...
use key_server_cluster::{
    ...
    math, DocumentKeyShare, Error, NodeId,
};
...
impl JobExecutor for SchnorrSigningJob {
    ...
    ) -> Result<SchnorrPartialSigningRequest, Error> {
    ...
    fn process_partial_request(
        &mut self,
        partial_request: SchnorrPartialSigningRequest,
    ) -> Result<JobPartialRequestAction<SchnorrPartialSigningResponse>, Error> {
        ...
        let combined_hash = math::combine_message_hash_with_public(
    ...
    fn check_partial_response(
        &mut self,
        _sender: &NodeId,
        partial_response: &SchnorrPartialSigningResponse,
    ...
    fn compute_response(
        &self,
        partial_responses: &BTreeMap<NodeId, SchnorrPartialSigningResponse>,
    ) -> Result<(Secret, Secret), Error> {
        let message_hash = self.message_hash.as_ref()
			.expect("compute_response is only called on master nodes; message_hash is filed in constructor on master nodes; qed");

        let signature_c =
            math::combine_message_hash_with_public(message_hash, &self.session_public)?;
        let signature_s = math::compute_schnorr_signature(
            partial_responses.values().map(|r| &r.partial_signature),
        )?;
