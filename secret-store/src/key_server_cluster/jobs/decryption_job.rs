...
use ethkey::{crypto::ecies::encrypt, Public, Secret};
use key_server_cluster::{
    jobs::job_session::{JobExecutor, JobPartialRequestAction, JobPartialResponseAction},
    math, DocumentKeyShare, EncryptedDocumentKeyShadow, Error, NodeId,
};
...
impl JobExecutor for DecryptionJob {
    ...
    ) -> Result<PartialDecryptionRequest, Error> {
    ...
    fn process_partial_request(
        &mut self,
        partial_request: PartialDecryptionRequest,
    ) -> Result<JobPartialRequestAction<PartialDecryptionResponse>, Error> {
        ...
        let other_id_numbers = partial_request
            .other_nodes_ids
            .iter()
            .map(|n| &key_version.id_numbers[n]);
        let node_shadow = math::compute_node_shadow(
            &key_version.secret_share,
            &self_id_number,
            other_id_numbers,
        )?;
        let decrypt_shadow = if partial_request.is_shadow_decryption {
            Some(math::generate_random_scalar()?)
        } else {
            None
        };
        let common_point = self
            ...
            .common_point
            .as_ref()
            .expect("DecryptionJob is only created when common_point is known; qed");
        let (shadow_point, decrypt_shadow) = math::compute_node_shadow_point(
            &self.access_key,
            &common_point,
            &node_shadow,
            decrypt_shadow,
        ...
        Ok(JobPartialRequestAction::Respond(
            PartialDecryptionResponse {
                ...
                decrypt_shadow: match decrypt_shadow.clone() {
                    None => None,
                    Some(decrypt_shadow) => {
                        Some(encrypt(&self.requester, &DEFAULT_MAC, &**decrypt_shadow)?)
                    }
                },
            },
        ))
    ...
    fn check_partial_response(
        &mut self,
        _sender: &NodeId,
        partial_response: &PartialDecryptionResponse,
    ...
    fn compute_response(
        &self,
        partial_responses: &BTreeMap<NodeId, PartialDecryptionResponse>,
        ...
        let common_point = self
            ...
            .as_ref()
            .expect("DecryptionJob is only created when common_point is known; qed");
        let encrypted_point = self
            .key_share
            .encrypted_point
            .as_ref()
            .expect("DecryptionJob is only created when encrypted_point is known; qed");
        let joint_shadow_point =
            math::compute_joint_shadow_point(partial_responses.values().map(|s| &s.shadow_point))?;
        let decrypted_secret = math::decrypt_with_joint_shadow(
        ...
        Ok(EncryptedDocumentKeyShadow {
            ...
            common_point: if is_shadow_decryption {
                Some(math::make_common_shadow_point(
                    self.key_share.threshold,
                    common_point.clone(),
                )?)
            } else {
