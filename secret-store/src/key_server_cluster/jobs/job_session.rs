...
pub trait JobExecutor {
    ...
    ) -> Result<Self::PartialJobRequest, Error>;
    /// Process partial request.
    fn process_partial_request(
        &mut self,
        partial_request: Self::PartialJobRequest,
    ) -> Result<JobPartialRequestAction<Self::PartialJobResponse>, Error>;
    /// Check partial response of given node.
    fn check_partial_response(
        &mut self,
        sender: &NodeId,
        partial_response: &Self::PartialJobResponse,
    ) -> Result<JobPartialResponseAction, Error>;
    /// Compute final job response.
    fn compute_response(
        &self,
        partial_responses: &BTreeMap<NodeId, Self::PartialJobResponse>,
    ) -> Result<Self::JobResponse, Error>;
}
...
pub trait JobTransport {
    ...
    fn send_partial_request(
        &self,
        node: &NodeId,
        request: Self::PartialJobRequest,
    ) -> Result<(), Error>;
    /// Send partial request to given node.
    fn send_partial_response(
        &self,
        node: &NodeId,
        response: Self::PartialJobResponse,
    ) -> Result<(), Error>;
}
...
pub struct JobSession<Executor: JobExecutor, Transport>
where
    Transport: JobTransport<
        PartialJobRequest = Executor::PartialJobRequest,
        PartialJobResponse = Executor::PartialJobResponse,
    >,
{
    /// Session meta.
    meta: SessionMeta,
...
impl<Executor, Transport> JobSession<Executor, Transport>
where
    ...
    >,
{
    ...
    pub fn initialize(
        &mut self,
        nodes: BTreeSet<NodeId>,
        self_response: Option<Executor::PartialJobResponse>,
        broadcast_self_response: bool,
    ) -> Result<Option<Executor::PartialJobResponse>, Error> {
        debug_assert!(self.meta.self_node_id == self.meta.master_node_id);

        if nodes.len() < self.meta.threshold + 1 {
            return Err(
                if self.meta.configured_nodes_count < self.meta.threshold + 1 {
        ...
        let self_response = match self_response {
            ...
            None if waits_for_self => {
                let partial_request = self
                    .executor
                    .prepare_partial_request(&self.meta.self_node_id, &active_data.requests)?;
        ...
        for node in nodes.iter().filter(|n| **n != self.meta.self_node_id) {
            if self.data.state == JobSessionState::Active {
                self.transport.send_partial_request(
                    node,
                    self.executor.prepare_partial_request(node, &nodes)?,
                )?;
            }
    ...
    pub fn on_partial_request(
        &mut self,
        node: &NodeId,
        request: Executor::PartialJobRequest,
    ) -> Result<JobPartialRequestAction<Executor::PartialJobResponse>, Error> {
        ...
        self.transport
            .send_partial_response(node, partial_response)?;
        Ok(partial_request_action)
    }

    /// When partial request is received by master node.
    pub fn on_partial_response(
        &mut self,
        node: &NodeId,
        response: Executor::PartialJobResponse,
    ) -> Result<(), Error> {
        if self.meta.self_node_id != self.meta.master_node_id {
            return Err(Error::InvalidMessage);
        }
        ...
        match self.executor.check_partial_response(node, &response)? {
            ...
            JobPartialResponseAction::Reject => {
                ...
                if active_data.requests.len() + active_data.responses.len()
                    >= self.meta.threshold + 1
                {
                    return Ok(());
                }
    ...
    pub fn on_node_error(&mut self, node: &NodeId, error: Error) -> Result<(), Error> {
        ...
        if let Some(active_data) = self.data.active_data.as_mut() {
            ...
            if active_data.requests.remove(node) || active_data.responses.remove(node).is_some() {
                active_data
                    .rejects
                    .insert(node.clone(), !error.is_non_fatal());
                if self.data.state == JobSessionState::Finished
                    && active_data.responses.len() < self.meta.threshold + 1
                {
                    self.data.state = JobSessionState::Active;
                }
                if active_data.requests.len() + active_data.responses.len()
...
pub mod tests {
    use super::{
        JobExecutor, JobPartialRequestAction, JobPartialResponseAction, JobSession,
        JobSessionState, JobTransport,
    };
    use ethkey::Public;
    ...
    impl JobExecutor for SquaredSumJobExecutor {
        ...
        fn process_partial_request(
            &mut self,
            r: u32,
        ) -> Result<JobPartialRequestAction<u32>, Error> {
            if r <= 10 {
                Ok(JobPartialRequestAction::Respond(r * r))
            } else {
        ...
        fn compute_response(&self, r: &BTreeMap<NodeId, u32>) -> Result<u32, Error> {
            Ok(r.values().fold(0, |v1, v2| v1 + v2))
        }
    }
    ...
    impl<T, U> JobTransport for DummyJobTransport<T, U> {
        ...
        fn send_partial_request(&self, node: &NodeId, request: T) -> Result<(), Error> {
            self.requests.lock().push_back((node.clone(), request));
            Ok(())
        }
        fn send_partial_response(&self, node: &NodeId, response: U) -> Result<(), Error> {
            self.responses.lock().push_back((node.clone(), response));
            Ok(())
        }
    }
    ...
    pub fn make_master_session_meta(threshold: usize) -> SessionMeta {
        SessionMeta {
            ...
            master_node_id: NodeId::from(1),
            self_node_id: NodeId::from(1),
            threshold: threshold,
    ...
    fn job_initialize_fails_if_not_enough_nodes_for_threshold_total() {
        let mut job = JobSession::new(
            make_master_session_meta(1),
            SquaredSumJobExecutor,
            DummyJobTransport::default(),
        );
        job.meta.configured_nodes_count = 1;
        assert_eq!(
            job.initialize(vec![Public::from(1)].into_iter().collect(), None, false)
                .unwrap_err(),
            Error::ConsensusUnreachable
        );
    }
    ...
    fn job_initialization_does_not_leads_to_finish_if_single_other_node_is_required() {
        ...
        job.initialize(vec![Public::from(2)].into_iter().collect(), None, false)
            .unwrap();
    ...
    fn job_request_fails_if_comes_from_non_master_node() {
        let mut job = JobSession::new(
            make_slave_session_meta(0),
            SquaredSumJobExecutor,
            DummyJobTransport::default(),
        );
        assert_eq!(
            job.on_partial_request(&NodeId::from(3), 2).unwrap_err(),
    ...
    fn job_request_fails_if_comes_to_master_node() {
        ...
        assert_eq!(
            job.on_partial_request(&NodeId::from(1), 2).unwrap_err(),
            Error::InvalidMessage
        );
    }
    ...
    fn job_response_fails_if_comes_to_slave_node() {
        ...
        assert_eq!(
            job.on_partial_response(&NodeId::from(1), 2).unwrap_err(),
            Error::InvalidMessage
        );
    }
    ...
    fn job_response_fails_if_comes_to_failed_state() {
        ...
        job.on_session_timeout().unwrap_err();
        assert_eq!(
            job.on_partial_response(&NodeId::from(2), 2).unwrap_err(),
            Error::InvalidStateForRequest
        );
    }
    ...
    fn job_response_fails_if_comes_from_unknown_node() {
        ...
        job.initialize(vec![Public::from(2)].into_iter().collect(), None, false)
            .unwrap();
        assert_eq!(
            job.on_partial_response(&NodeId::from(3), 2).unwrap_err(),
    ...
    fn job_response_leads_to_failure_if_too_few_nodes_left() {
        ...
        job.initialize(
            vec![Public::from(1), Public::from(2)].into_iter().collect(),
            None,
            false,
        )
        .unwrap();
        assert_eq!(job.state(), JobSessionState::Active);
        assert_eq!(
            job.on_partial_response(&NodeId::from(2), 3).unwrap_err(),
            Error::ConsensusUnreachable
        );
    ...
    fn job_response_succeeds() {
        let mut job = JobSession::new(
            make_master_session_meta(2),
            SquaredSumJobExecutor,
            DummyJobTransport::default(),
        );
        job.initialize(
            vec![Public::from(1), Public::from(2), Public::from(3)]
                .into_iter()
                .collect(),
            None,
            false,
    ...
    fn job_node_error_ignored_when_slave_disconnects_from_slave() {
        let mut job = JobSession::new(
            make_slave_session_meta(1),
            SquaredSumJobExecutor,
            DummyJobTransport::default(),
        );
        assert_eq!(job.state(), JobSessionState::Inactive);
        job.on_node_error(&NodeId::from(3), Error::AccessDenied)
            .unwrap();
        assert_eq!(job.state(), JobSessionState::Inactive);
    }
    ...
    fn job_node_error_leads_to_fail_when_slave_disconnects_from_master() {
        ...
        assert_eq!(job.state(), JobSessionState::Inactive);
        assert_eq!(
            job.on_node_error(&NodeId::from(1), Error::AccessDenied)
                .unwrap_err(),
            Error::ConsensusUnreachable
        );
        assert_eq!(job.state(), JobSessionState::Failed);
    ...
    fn job_node_error_ignored_when_disconnects_from_rejected() {
        ...
        job.on_node_error(&NodeId::from(2), Error::AccessDenied)
            .unwrap();
        assert_eq!(job.state(), JobSessionState::Active);
    }
    ...
    fn job_fails_with_temp_error_if_more_than_half_nodes_respond_with_temp_error() {
        ...
        .unwrap();
        job.on_node_error(&NodeId::from(2), Error::NodeDisconnected)
            .unwrap();
        assert_eq!(
            job.on_node_error(&NodeId::from(3), Error::NodeDisconnected)
                .unwrap_err(),
            Error::ConsensusTemporaryUnreachable
        );
    }
    ...
    fn job_fails_with_temp_error_when_temp_error_is_reported_by_master_node() {
        let mut job = JobSession::new(
            make_slave_session_meta(2),
            SquaredSumJobExecutor,
            DummyJobTransport::default(),
        );
        assert_eq!(
            job.on_node_error(&NodeId::from(1), Error::NodeDisconnected)
                .unwrap_err(),
            Error::ConsensusTemporaryUnreachable
        );
    }
