...
use key_server_cluster::{
    jobs::job_session::{
        JobExecutor, JobPartialRequestAction, JobSession, JobSessionState, JobTransport,
    },
    message::ConsensusMessage,
    Error, NodeId, Requester, SessionMeta,
};
...
pub struct ConsensusSession<
    ConsensusExecutor: JobExecutor<PartialJobResponse = bool>,
    ConsensusTransport: JobTransport<
        PartialJobRequest = ConsensusExecutor::PartialJobRequest,
        PartialJobResponse = ConsensusExecutor::PartialJobResponse,
    >,
    ComputationExecutor: JobExecutor,
    ComputationTransport: JobTransport<
        PartialJobRequest = ComputationExecutor::PartialJobRequest,
        PartialJobResponse = ComputationExecutor::PartialJobResponse,
    >,
> {
...
pub struct ConsensusSessionParams<
    ConsensusExecutor: JobExecutor<PartialJobResponse = bool>,
    ConsensusTransport: JobTransport<
        PartialJobRequest = ConsensusExecutor::PartialJobRequest,
        PartialJobResponse = ConsensusExecutor::PartialJobResponse,
...
impl<ConsensusExecutor, ConsensusTransport, ComputationExecutor, ComputationTransport>
    ConsensusSession<
        ConsensusExecutor,
        ConsensusTransport,
        ComputationExecutor,
        ComputationTransport,
    >
where
    ConsensusExecutor: JobExecutor<PartialJobResponse = bool, JobResponse = BTreeSet<NodeId>>,
    ConsensusTransport: JobTransport<
        PartialJobRequest = ConsensusExecutor::PartialJobRequest,
        PartialJobResponse = ConsensusExecutor::PartialJobResponse,
    >,
    ...
    >,
{
    ...
    pub fn new(
        params: ConsensusSessionParams<ConsensusExecutor, ConsensusTransport>,
    ) -> Result<Self, Error> {
        let consensus_job = JobSession::new(
            params.meta.clone(),
            params.consensus_executor,
            params.consensus_transport,
        );
    ...
    pub fn on_consensus_partial_request(
        &mut self,
        sender: &NodeId,
        request: ConsensusExecutor::PartialJobRequest,
    ) -> Result<(), Error> {
    ...
    pub fn on_consensus_partial_response(
        &mut self,
        sender: &NodeId,
        response: bool,
    ) -> Result<(), Error> {
    ...
    pub fn disseminate_jobs(
        ...
        transport: ComputationTransport,
        broadcast_self_response: bool,
    ) -> Result<Option<ComputationExecutor::PartialJobResponse>, Error> {
        ...
        let computation_result =
            computation_job.initialize(consensus_group, None, broadcast_self_response);
    ...
    pub fn on_job_request(
        &mut self,
        node: &NodeId,
        request: ComputationExecutor::PartialJobRequest,
        executor: ComputationExecutor,
        transport: ComputationTransport,
    ) -> Result<JobPartialRequestAction<ComputationExecutor::PartialJobResponse>, Error> {
    ...
    pub fn on_job_response(
        &mut self,
        node: &NodeId,
        response: ComputationExecutor::PartialJobResponse,
    ) -> Result<(), Error> {
            ...
            ConsensusSessionState::WaitingForPartialResults => {
                ...
                if !is_computation_node {
                    ...
                    let is_consensus_established =
                        self.consensus_job.state() == JobSessionState::Finished;
                    (is_consensus_established, consensus_result)
                }
            }
            // in all other cases - just ignore error
            ConsensusSessionState::WaitingForInitialization
            | ConsensusSessionState::Failed
            | ConsensusSessionState::Finished => (false, Ok(())),
        };
    ...
    pub fn on_session_timeout(&mut self) -> Result<bool, Error> {
        match self.state {
            ...
            | ConsensusSessionState::ConsensusEstablished => {
                ...
                self.state = ConsensusSessionState::EstablishingConsensus;
                return self
                    .process_result(Err(Error::ConsensusTemporaryUnreachable))
                    .map(|_| unreachable!());
            }
            // in all other cases - just ignore error
        ...
        let timeouted_nodes = self
            .computation_job
            .as_ref()
            .expect(
                "WaitingForPartialResults state is only set when computation_job is created; qed",
            )
            .requests()
            .clone();
        ...
        for timeouted_node in timeouted_nodes {
            let timeout_result = self
                .consensus_job
                .on_node_error(&timeouted_node, Error::NodeDisconnected);
            self.state = ConsensusSessionState::EstablishingConsensus;
            self.process_result(timeout_result)?;
        }
...
impl<ConsensusExecutor, ConsensusTransport, ComputationExecutor, ComputationTransport>
    ...
    >
where
    ConsensusExecutor: JobExecutor<
        PartialJobRequest = Requester,
        PartialJobResponse = bool,
        JobResponse = BTreeSet<NodeId>,
    >,
    ConsensusTransport: JobTransport<
        PartialJobRequest = ConsensusExecutor::PartialJobRequest,
        PartialJobResponse = ConsensusExecutor::PartialJobResponse,
    ...
    >,
{
    ...
    pub fn on_consensus_message(
        ...
        let consensus_result = match message {
            &ConsensusMessage::InitializeConsensusSession(ref message) => self
                .consensus_job
                .on_partial_request(sender, message.requester.clone().into())
                .map(|_| ()),
            &ConsensusMessage::ConfirmConsensusInitialization(ref message) => self
                .consensus_job
                .on_partial_response(sender, message.is_confirmed),
        };
        self.process_result(consensus_result)
    }
}
...
mod tests {
    ...
    use ethkey::{public_to_address, sign, Generator, KeyPair, Random};
    use key_server_cluster::{
        jobs::{
            job_session::tests::{
                make_master_session_meta, make_slave_session_meta, DummyJobTransport,
                SquaredSumJobExecutor,
            },
            key_access_job::KeyAccessJob,
        },
        message::{ConfirmConsensusInitialization, ConsensusMessage, InitializeConsensusSession},
        DummyAclStorage, Error, NodeId, Requester, SessionId,
    };
    use std::sync::Arc;

    type SquaredSumConsensusSession = ConsensusSession<
        KeyAccessJob,
        DummyJobTransport<Requester, bool>,
        SquaredSumJobExecutor,
        DummyJobTransport<u32, u32>,
    >;

    fn make_master_consensus_session(
        threshold: usize,
        requester: Option<KeyPair>,
        acl_storage: Option<DummyAclStorage>,
    ) -> SquaredSumConsensusSession {
        let secret = requester
            .map(|kp| kp.secret().clone())
            .unwrap_or(Random.generate().unwrap().secret().clone());
        SquaredSumConsensusSession::new(ConsensusSessionParams {
            meta: make_master_session_meta(threshold),
            consensus_executor: KeyAccessJob::new_on_master(
                SessionId::default(),
                Arc::new(acl_storage.unwrap_or(DummyAclStorage::default())),
    ...
    fn make_slave_consensus_session(
        threshold: usize,
        acl_storage: Option<DummyAclStorage>,
    ) -> SquaredSumConsensusSession {
        SquaredSumConsensusSession::new(ConsensusSessionParams {
            meta: make_slave_session_meta(threshold),
            consensus_executor: KeyAccessJob::new_on_slave(
                SessionId::default(),
                Arc::new(acl_storage.unwrap_or(DummyAclStorage::default())),
            ),
            consensus_transport: DummyJobTransport::default(),
    ...
    fn consensus_session_consensus_is_not_reached_when_initializes_with_non_zero_threshold() {
        ...
        session
            .initialize(vec![NodeId::from(1), NodeId::from(2)].into_iter().collect())
            .unwrap();
        assert_eq!(
            session.state(),
            ConsensusSessionState::EstablishingConsensus
        ...
        session
            .on_consensus_message(
                &NodeId::from(2),
                &ConsensusMessage::ConfirmConsensusInitialization(ConfirmConsensusInitialization {
                    is_confirmed: true,
                }),
            )
            .unwrap();
    ...
    fn consensus_session_consensus_is_failed_by_slave_node() {
        ...
        assert_eq!(
            session
                .on_consensus_message(
                    &NodeId::from(2),
                    &ConsensusMessage::ConfirmConsensusInitialization(
    ...
    fn consensus_session_job_dissemination_fails_if_consensus_is_not_reached() {
        ...
        assert_eq!(
            session
                .disseminate_jobs(SquaredSumJobExecutor, DummyJobTransport::default(), false)
                .unwrap_err(),
            ...
            .on_consensus_message(
                ...
                &ConsensusMessage::InitializeConsensusSession(InitializeConsensusSession {
                    requester: Requester::Signature(
                        sign(Random.generate().unwrap().secret(), &SessionId::default()).unwrap(),
                    )
                    .into(),
    ...
    fn consensus_session_computation_response_is_ignored_when_consensus_is_not_reached() {
        let mut session = make_master_consensus_session(1, None, None);
        assert_eq!(
            session.on_job_response(&NodeId::from(2), 4).unwrap_err(),
            Error::InvalidStateForRequest
        );
    }
    ...
    fn consessus_session_completion_is_ignored_when_received_from_non_master_node() {
        let mut session = make_slave_consensus_session(0, None);
        assert_eq!(
            session.on_session_completed(&NodeId::from(3)).unwrap_err(),
            Error::InvalidMessage
        );
    }
    ...
    fn consessus_session_completion_is_ignored_when_consensus_is_not_established() {
        let mut session = make_slave_consensus_session(0, None);
        assert_eq!(
            session.on_session_completed(&NodeId::from(1)).unwrap_err(),
            Error::InvalidStateForRequest
        );
    }
    ...
    fn consensus_session_fails_if_node_error_received_by_uninitialized_master() {
        let mut session = make_master_consensus_session(0, None, None);
        assert_eq!(
            session.on_node_error(&NodeId::from(2), Error::AccessDenied),
    ...
    fn consensus_session_fails_if_node_error_received_by_uninitialized_slave_from_master() {
        let mut session = make_slave_consensus_session(0, None);
        assert_eq!(
            session.on_node_error(&NodeId::from(1), Error::AccessDenied),
            Err(Error::ConsensusUnreachable)
        );
        assert_eq!(session.state(), ConsensusSessionState::Failed);
    }
    ...
    fn consensus_sessions_fails_with_temp_error_if_node_error_received_by_uninitialized_slave_from_master(
        ...
        assert_eq!(
            session
                .on_node_error(&NodeId::from(1), Error::NodeDisconnected)
                .unwrap_err(),
            Error::ConsensusTemporaryUnreachable
        );
    }
    ...
    fn consensus_session_continues_if_node3_error_received_by_master_after_consensus_established_and_enough_nodes_left(
        ...
        session
            ...
            .unwrap();
        assert_eq!(
            session.on_node_error(&NodeId::from(3), Error::AccessDenied),
            Ok(false)
        );
        assert_eq!(session.state(), ConsensusSessionState::ConsensusEstablished);
    }
    ...
    fn consensus_session_continues_if_node_error_received_from_slave_not_participating_in_computation(
        ...
        assert_eq!(
            session.on_node_error(&NodeId::from(4), Error::AccessDenied),
            Ok(false)
        );
        assert_eq!(
            session.state(),
    ...
    fn consensus_session_complete_2_of_4_after_restart() {
        ...
        assert_eq!(
            session
                .on_node_error(&NodeId::from(2), Error::AccessDenied)
                .unwrap(),
            true
        );
        assert_eq!(session.state(), ConsensusSessionState::ConsensusEstablished);
        ...
        assert_eq!(
            session
                .on_node_error(&NodeId::from(3), Error::AccessDenied)
                .unwrap(),
            false
        );
        assert_eq!(
