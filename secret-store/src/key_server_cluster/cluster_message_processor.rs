...
use key_server_cluster::{
    ...
    cluster_connections::{Connection, ConnectionProvider},
    cluster_sessions::{
        create_cluster_view, AdminSession, ClusterSession, ClusterSessions,
        ClusterSessionsContainer,
    },
    cluster_sessions_creator::{ClusterSessionCreator, IntoSessionId},
...
pub trait MessageProcessor: Send + Sync {
    ...
    fn start_servers_set_change_session(
        &self,
...
impl SessionsMessageProcessor {
        ...
        let session = match session {
            ...
            Err(error) => {
                ...
                if !message.is_error_message() {
                    ...
                    connection.send_message(SC::make_error_message(
                        session_id,
                        session_nonce,
                        error,
                    ));
        ...
        Message: IntoSessionId<S::Id>,
    {
        ...
        // get or create new session, if required
        let session_id = message.into_session_id().expect(
            "into_session_id fails for cluster messages only;
    ...
    fn process_cluster_message(&self, connection: Arc<dyn Connection>, message: ClusterMessage) {
        match message {
            ClusterMessage::KeepAlive(_) => {
                let msg = Message::Cluster(ClusterMessage::KeepAliveResponse(
                    message::KeepAliveResponse { session_id: None },
                ));
            ...
            _ => {
                warn!(target: "secretstore_net", "{}: received unexpected message {} from node {} at {}",
    ...
    fn process_connection_message(&self, connection: Arc<dyn Connection>, message: Message) {
        ...
        match message {
            Message::Generation(message) => self
                .process_message(
                    &self.sessions.generation_sessions,
                    connection,
            ...
            Message::Encryption(message) => self
                .process_message(
                    &self.sessions.encryption_sessions,
                    connection,
            ...
            Message::Decryption(message) => self
                .process_message(
                    &self.sessions.decryption_sessions,
                    connection,
            ...
            Message::SchnorrSigning(message) => self
                .process_message(
                    &self.sessions.schnorr_signing_sessions,
                    connection,
            ...
            Message::EcdsaSigning(message) => self
                .process_message(
                    &self.sessions.ecdsa_signing_sessions,
                    connection,
            ...
            Message::ServersSetChange(message) => {
                ...
                let is_initialization_message = message.is_initialization_message();
                let session =
                    self.process_message(&self.sessions.admin_sessions, connection, message);
        ...
        if let Some(session) = session {
            ...
            if is_master_node && session.is_finished() {
                ...
                match session.wait() {
                    ...
                    Ok(None) => unreachable!(
                        "is_master_node; session is finished;
