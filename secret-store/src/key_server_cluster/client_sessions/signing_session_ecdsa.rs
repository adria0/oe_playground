...
mod tests {
    use ethereum_types::H256;
    use ethkey::{self, public_to_address, verify_public, Generator, Public, Random};
    use key_server_cluster::{
        cluster::tests::MessageLoop as ClusterMessageLoop,
        generation_session::tests::MessageLoop as GenerationMessageLoop,
        signing_session_ecdsa::SessionImpl, Error, KeyStorage, SessionId,
    };
    use std::sync::Arc;
    ...
    fn failed_gen_ecdsa_sign_session_when_threshold_is_too_low() {
        ...
        for &(threshold, num_nodes) in &test_cases {
            assert_eq!(
                MessageLoop::new(num_nodes, threshold)
                    .unwrap()
                    .init()
                    .unwrap_err(),
    ...
    fn ecdsa_complete_signing_session_with_single_node_failing() {
        ...
        ml.0.acl_storage(1)
            .prohibit(public_to_address(&requester), Default::default());
