...
use key_server_cluster::{
    ...
    jobs::{
        ...
        decryption_job::{DecryptionJob, PartialDecryptionRequest, PartialDecryptionResponse},
        job_session::{JobSession, JobSessionState, JobTransport},
        key_access_job::KeyAccessJob,
    },
    message::{
        ConfirmConsensusInitialization, ConsensusMessage, DecryptionConsensusMessage,
        DecryptionMessage, DecryptionSessionCompleted, DecryptionSessionDelegation,
        DecryptionSessionDelegationCompleted, DecryptionSessionError, InitializeConsensusSession,
        Message, PartialDecryption, RequestPartialDecryption,
    },
    AclStorage, DocumentKeyShare, EncryptedDocumentKeyShadow, Error, NodeId, Requester, SessionId,
    SessionMeta,
};
...
type DecryptionConsensusSession = ConsensusSession<
    KeyAccessJob,
...
impl SessionImpl {
    ...
    ) -> Option<Result<EncryptedDocumentKeyShadow, Error>> {
        Self::wait_session(&self.core.completed, &self.data, timeout, |data| {
            data.result.clone()
        })
    }
    ...
    pub fn broadcast_shadows(&self) -> Option<BTreeMap<NodeId, Vec<u8>>> {
        ...
        if data.result.is_none()
            || (data.is_broadcast_session, data.is_shadow_decryption) != (Some(true), Some(true))
        {
            return None;
        }
    ...
    pub fn on_session_delegated(
        &self,
        sender: &NodeId,
        message: &DecryptionSessionDelegation,
    ) -> Result<(), Error> {
        ...
        self.initialize(
            message.origin.clone().map(Into::into),
            message.version.clone().into(),
            message.is_shadow_decryption,
            message.is_broadcast_session,
        )
    }
    ...
    pub fn on_session_delegation_completed(
        &self,
        sender: &NodeId,
        message: &DecryptionSessionDelegationCompleted,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        debug_assert!(self.core.access_key == *message.sub_session);
    ...
    pub fn on_consensus_message(
        &self,
        sender: &NodeId,
        message: &DecryptionConsensusMessage,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        debug_assert!(self.core.access_key == *message.sub_session);
    ...
    pub fn on_partial_decryption_requested(
        &self,
        sender: &NodeId,
        ...
        let key_version = key_share
            .version(data.version.as_ref().ok_or(Error::InvalidMessage)?)?
            .hash
            .clone();
        let requester_public = data
            .consensus_session
            .consensus_job()
            .executor()
            .requester()
        ...
        let decryption_job = DecryptionJob::new_on_slave(
            self.core.meta.self_node_id.clone(),
            self.core.access_key.clone(),
            requester_public.clone(),
            key_share.clone(),
            key_version,
        )?;
        ...
        // respond to request
        let partial_decryption = data.consensus_session.on_job_request(
            sender,
            ...
            decryption_job,
            decryption_transport,
        )?;

        // ...and prepare decryption job session if we need to broadcast result
        if message.is_broadcast_session {
            let consensus_group: BTreeSet<_> =
                message.nodes.iter().cloned().map(Into::into).collect();
            let broadcast_decryption_job = DecryptionJob::new_on_master(
                self.core.meta.self_node_id.clone(),
                self.core.access_key.clone(),
                requester_public,
                key_share.clone(),
            ...
            Self::create_broadcast_decryption_job(
                ...
                &mut *data,
                consensus_group,
                broadcast_decryption_job,
                message.request_id.clone().into(),
                Some(partial_decryption.take_response()),
            )?;
        }
        ...
        let result = if is_master_node {
            ...
            match data.broadcast_job_session.as_mut() {
                Some(broadcast_job_session) => {
                    broadcast_job_session.on_partial_response(
                        sender,
                        PartialDecryptionResponse {
                            request_id: message.request_id.clone().into(),
                            shadow_point: message.shadow_point.clone().into(),
    ...
    fn process_node_error(&self, node: Option<&NodeId>, error: Error) -> Result<(), Error> {
        ...
        match {
            ...
            Ok(true) => {
                ...
                let disseminate_result = Self::disseminate_jobs(
                    &self.core,
                    &mut *data,
                    &version,
                    is_shadow_decryption,
                ...
                match disseminate_result {
                    ...
                    Err(err) => {
                        warn!(
                            "{}: decryption session failed with error: {:?} from {:?}",
    ...
    fn disseminate_jobs(
        ...
        let requester = data
            ...
            .ok_or(Error::InvalidStateForRequest)?
            .clone();
        let requester_public = requester
            .public(&core.meta.id)
            .map_err(Error::InsufficientRequesterData)?;
        let consensus_group = data.consensus_session.select_consensus_group()?.clone();
        let decryption_job = DecryptionJob::new_on_master(
            core.meta.self_node_id.clone(),
            core.access_key.clone(),
            requester_public.clone(),
            key_share.clone(),
            key_version,
            is_shadow_decryption,
        ...
        let self_response = data.consensus_session.disseminate_jobs(
            decryption_job,
            decryption_transport,
            is_broadcast_session,
        )?;
    ...
    fn create_broadcast_decryption_job(
        core: &SessionCore,
        data: &mut SessionData,
        mut consensus_group: BTreeSet<NodeId>,
        mut job: DecryptionJob,
        request_id: Secret,
        self_response: Option<PartialDecryptionResponse>,
    ) -> Result<(), Error> {
        ...
        let mut job_session = JobSession::new(
            SessionMeta {
        ...
        job_session.initialize(
            consensus_group,
            self_response,
            core.meta.self_node_id != core.meta.master_node_id,
        )?;
    ...
    fn set_decryption_result(
        core: &SessionCore,
        data: &mut SessionData,
        result: Result<EncryptedDocumentKeyShadow, Error>,
    ) {
        if let Some(DelegationStatus::DelegatedFrom(master, nonce)) = data.delegation_status.take()
        {
            ...
            let _ = match result.as_ref() {
                Ok(document_key) => core.cluster.send(
                    &master,
                    Message::Decryption(DecryptionMessage::DecryptionSessionDelegationCompleted(
                        DecryptionSessionDelegationCompleted {
                ...
                Err(error) => core.cluster.send(
                    &master,
                    Message::Decryption(DecryptionMessage::DecryptionSessionError(
...
impl ClusterSession for SessionImpl {
    ...
    fn on_session_error(&self, node: &NodeId, error: Error) {
        ...
        if is_fatal || is_this_node_error {
            ...
            // => or broadcast error
            let message = Message::Decryption(DecryptionMessage::DecryptionSessionError(
                DecryptionSessionError {
                    session: self.core.meta.id.clone().into(),
                    sub_session: self.core.access_key.clone().into(),
                    session_nonce: self.core.nonce,
...
impl JobTransport for DecryptionConsensusTransport {
    ...
    fn send_partial_request(&self, node: &NodeId, request: Requester) -> Result<(), Error> {
        ...
        self.cluster.send(
            node,
            Message::Decryption(DecryptionMessage::DecryptionConsensusMessage(
...
impl JobTransport for DecryptionJobTransport {
    ...
    fn send_partial_request(
        ...
        if !self.is_broadcast_transport {
            self.cluster.send(
                node,
                Message::Decryption(DecryptionMessage::RequestPartialDecryption(
    ...
    fn send_partial_response(
        &self,
        node: &NodeId,
        response: PartialDecryptionResponse,
    ) -> Result<(), Error> {
        if !self.is_broadcast_transport || *node != self.master_node_id {
            self.cluster.send(
                node,
                Message::Decryption(DecryptionMessage::PartialDecryption(PartialDecryption {
                    session: self.id.clone().into(),
                    sub_session: self.access_key.clone().into(),
                    session_nonce: self.nonce,
...
mod tests {
    use acl_storage::DummyAclStorage;
    use ethkey::{self, public_to_address, Generator, KeyPair, Public, Random, Secret};
    use key_server_cluster::{
        cluster::tests::DummyCluster,
        cluster_sessions::ClusterSession,
        decryption_session::{SessionImpl, SessionParams},
        jobs::consensus_session::ConsensusSessionState,
        math,
        message::{self, DecryptionMessage, Message},
        DocumentKeyShare, DocumentKeyShareVersion, EncryptedDocumentKeyShadow, Error, NodeId,
    ...
    fn prepare_decryption_sessions() -> (
        ...
        let secret_shares: Vec<Secret> = vec![
            "834cb736f02d9c968dfaf0c37658a1d86ff140554fc8b59c9fdad5a8cf810eec"
                .parse()
                .unwrap(),
            "5a3c1d90fafafa66bb808bcc464354a98b05e6b2c95b5f609d4511cdd1b17a0b"
                .parse()
                .unwrap(),
            "71bf61e7848e08e3a8486c308ce521bdacfebcf9116a0151447eb301f3a2d0e9"
                .parse()
                .unwrap(),
            "80c0e5e2bea66fa9b2e07f7ce09630a9563e8242446d5ee63221feb09c4338f4"
                .parse()
                .unwrap(),
            "c06546b5669877ba579ca437a5602e89425c53808c708d44ccd6afcaa4610fad"
                .parse()
                .unwrap(),
        ];
    ...
    fn fails_to_initialize_if_does_not_have_a_share() {
        ...
        .unwrap();
        assert_eq!(
            session.initialize(Default::default(), Default::default(), false, false),
    ...
    fn fails_to_initialize_when_already_initialized() {
        ...
        assert_eq!(
            sessions[0]
                .initialize(Default::default(), Default::default(), false, false)
                .unwrap(),
            ()
        );
        assert_eq!(
            ...
            sessions[0]
                .on_consensus_message(
                    ...
                    &message::DecryptionConsensusMessage {
                        ...
                        session_nonce: 0,
                        origin: None,
                        message: message::ConsensusMessage::InitializeConsensusSession(
                            message::InitializeConsensusSession {
                                requester: Requester::Signature(
                                    ethkey::sign(
                                        Random.generate().unwrap().secret(),
                                        &SessionId::default()
                                    )
                                    .unwrap()
                                )
    ...
    fn complete_dec_session() {
        ...
        assert_eq!(
            sessions
                .iter()
                .filter(|s| s.state() == ConsensusSessionState::Finished)
                .count(),
            5
        );
        ...
        assert!(sessions
            .iter()
            .skip(1)
            .all(|s| s.decrypted_secret().is_none()));

        assert_eq!(
            sessions[0].decrypted_secret().unwrap().unwrap(),
    ...
    fn complete_shadow_dec_session() {
        ...
        // now let's try to do a decryption
        sessions[0]
            .initialize(Default::default(), Default::default(), true, false)
            .unwrap();
        ...
        let decrypt_shadows: Vec<_> = decrypted_secret
            .decrypt_shadows
            .unwrap()
            .into_iter()
            .map(|c| {
                Secret::from_slice(&decrypt(key_pair.secret(), &DEFAULT_MAC, &c).unwrap()).unwrap()
            })
            .collect();
        let decrypted_secret = math::decrypt_with_shadow_coefficients(
            decrypted_secret.decrypted_secret,
            decrypted_secret.common_point.unwrap(),
            decrypt_shadows,
        )
        .unwrap();
    ...
    fn failed_dec_session() {
        ...
        assert_eq!(
            sessions
                .iter()
                .filter(|s| s.state() == ConsensusSessionState::Failed)
                .count(),
            3
        );
    }
    ...
    fn decryption_message_fails_when_nonce_is_wrong() {
        ...
        assert_eq!(
            sessions[1].process_message(
                sessions[0].node(),
                &message::DecryptionMessage::DecryptionSessionCompleted(
                    message::DecryptionSessionCompleted {
                        session: SessionId::default().into(),
                        sub_session: sessions[0].access_key().clone().into(),
                        session_nonce: 10,
    ...
    fn decryption_works_when_delegated_to_other_node() {
        ...
        sessions[1]
            .delegate(
                sessions[0].core.meta.self_node_id.clone(),
                Default::default(),
                Default::default(),
                false,
                false,
        ...
        // 2) 1 session has decrypted key value
        assert_eq!(
            sessions[1].decrypted_secret().unwrap().unwrap(),
            EncryptedDocumentKeyShadow {
                decrypted_secret: SECRET_PLAIN.into(),
                common_point: None,
                decrypt_shadows: None,
    ...
    fn decryption_result_restored_on_all_nodes_if_broadcast_session_is_completed() {
        let (_, clusters, _, sessions) = prepare_decryption_sessions();
        sessions[0]
            .initialize(Default::default(), Default::default(), false, true)
            .unwrap();
        do_messages_exchange(&clusters, &sessions).unwrap();
        ...
        assert_eq!(
            ...
            sessions
                .iter()
                .skip(1)
                .filter(|s| s.decrypted_secret() == result)
                .count()
        );
        assert_eq!(
            1,
            sessions
                .iter()
                .skip(1)
                .filter(|s| s.decrypted_secret().is_none())
                .count()
        );
    }
    ...
    fn decryption_shadows_restored_on_all_nodes_if_shadow_broadcast_session_is_completed() {
        let (key_pair, clusters, _, sessions) = prepare_decryption_sessions();
        sessions[0]
            .initialize(Default::default(), Default::default(), true, true)
            .unwrap();
        do_messages_exchange(&clusters, &sessions).unwrap();
        ...
        assert_eq!(
            ...
            sessions
                .iter()
                .skip(1)
                .filter(|s| s.broadcast_shadows() == broadcast_shadows)
                .count()
        );
        assert_eq!(
            1,
            sessions
                .iter()
                .skip(1)
                .filter(|s| s.broadcast_shadows().is_none())
                .count()
        );
        ...
        assert_eq!(
            ...
            sessions
                .iter()
                .skip(1)
                .filter(|s| s.decrypted_secret() == Some(Ok(result.clone())))
                .count()
        );
        let decrypt_shadows: Vec<_> = result
            .decrypt_shadows
            .unwrap()
            .into_iter()
            .map(|c| {
    ...
    fn decryption_session_origin_is_known_to_all_initialized_nodes() {
        let (_, clusters, _, sessions) = prepare_decryption_sessions();
        sessions[0]
            .initialize(Some(1.into()), Default::default(), true, true)
            .unwrap();
        do_messages_exchange(&clusters, &sessions).unwrap();
