...
use key_server_cluster::{
    ...
    math,
    message::{
        CommonKeyData, ConfirmConsensusInitialization, ConsensusMessageOfShareAdd,
    ...
    Clone + JobTransport<PartialJobRequest = ServersSetChangeAccessRequest, PartialJobResponse = bool>
{
    ...
    fn set_master_data(
        ...
        version_holders: BTreeSet<NodeId>,
    ...
    T: SessionTransport,
{
    ...
    pub fn initialize(
        ...
        consensus_transport.set_master_data(
            consensus_group.clone(),
            version_holders.clone(),
            new_nodes_map.clone(),
        );
            ...
            let (version, new_nodes_map, consensus_group, version_holders) = match &message.message
            {
                &ConsensusMessageOfShareAdd::InitializeConsensusSession(ref message) => {
                    ...
                    (
                        Some(version),
                        Some(new_nodes_map),
                        Some(consensus_group),
                        Some(version_holders),
                    )
                }
        ...
        match has_this_version {
            true => {
                ...
                if key_version
                    .id_numbers
                    .keys()
                    .filter(|n| non_isolated_nodes.contains(n) && version_holders.contains(n))
                    .count()
                    >= new_nodes_map.len()
                {
                    return Err(Error::ConsensusUnreachable);
...
pub mod tests {
    ...
    impl AdminSessionAdapter<SessionImpl<IsolatedSessionTransport>> for Adapter {
        ...
        fn create(
            ...
            let key_version = key_storage
                .get(&meta.id)
                .unwrap()
                .map(|ks| ks.last_version().unwrap().hash);

            meta.self_node_id = *ml.node_key_pair(idx).public();
            SessionImpl::new(SessionParams {
                meta: meta.clone(),
                transport: IsolatedSessionTransport::new(
                    meta.id,
                    key_version,
                    1,
                    ml.cluster(idx).view().unwrap(),
                ),
                key_storage,
    ...
    fn node_add_fails_if_nodes_removed() {
        ...
        assert_eq!(
            MessageLoop::with_gml::<Adapter>(gml, master, Some(add), Some(remove), None)
                .run_at(master)
                .unwrap_err(),
    ...
    fn node_add_fails_if_no_nodes_added() {
        ...
        let master = gml.0.node(0);
        assert_eq!(
            MessageLoop::with_gml::<Adapter>(gml, master, Some(add), None, None)
                .run_at(master)
                .unwrap_err(),
            Error::ConsensusUnreachable
        );
    ...
    fn node_add_fails_if_started_without_signatures() {
        ...
        let master = gml.0.node(0);
        assert_eq!(
            MessageLoop::with_gml::<Adapter>(gml, master, Some(add), None, None).sessions[&master]
    ...
    fn nodes_add_to_the_node_with_obsolete_version() {
        ...
        ml.ml
            .replace_key_storage_of(&node_to_isolate, isolated_key_storage.clone());
        ml.sessions
            .get_mut(&node_to_isolate)
            .unwrap()
            .core
        ...
        let oldest_key_share = ml
            .ml
            .key_storage_of(oldest_nodes_set.iter().nth(0).unwrap())
            .get(&Default::default())
            .unwrap()
            .unwrap();
        ...
        debug_assert!(oldest_nodes_set.iter().all(|n| vec![
            ...
            .map(|v| v.hash)
            .collect::<Vec<_>>()));
        debug_assert!(::std::iter::once(&node_to_isolate).all(|n| vec![
            version_a.clone(),
            version_c.clone()
        ] == ml
            .ml
    ...
    fn nodes_add_fails_when_not_enough_share_owners_are_connected() {
        ...
        assert_eq!(
            MessageLoop::with_gml::<Adapter>(gml, master, Some(add), None, Some(isolate))
                .run_at(master)
                .unwrap_err(),
            Error::ConsensusUnreachable
        );
