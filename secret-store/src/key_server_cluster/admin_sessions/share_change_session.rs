...
use key_server_cluster::{
    ...
    jobs::{
        job_session::JobTransport, servers_set_change_access_job::ServersSetChangeAccessRequest,
    },
    math,
    message::{Message, ServersSetChangeMessage, ServersSetChangeShareAddMessage, ShareAddMessage},
    share_add_session::{
        SessionImpl as ShareAddSessionImpl, SessionParams as ShareAddSessionParams,
        SessionTransport as ShareAddSessionTransport,
    },
    Error, KeyStorage, NodeId, ServerKeyId, SessionId,
};
use std::{
...
impl ShareChangeSession {
    ...
    pub fn new(params: ShareChangeSessionParams) -> Result<Self, Error> {
        ...
        let version_holders = if !params.plan.version_holders.is_empty() {
            Some(params.plan.version_holders)
        } else {
            None
        };
...
impl ShareAddSessionTransport for ShareChangeTransport {
    ...
    fn set_master_data(
        ...
        _version_holders: BTreeSet<NodeId>,
