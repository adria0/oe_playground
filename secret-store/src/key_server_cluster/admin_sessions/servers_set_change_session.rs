...
use key_server_cluster::{
    ...
    jobs::{
        consensus_session::{ConsensusSession, ConsensusSessionParams, ConsensusSessionState},
        job_session::JobTransport,
        servers_set_change_access_job::{ServersSetChangeAccessJob, ServersSetChangeAccessRequest},
        unknown_sessions_job::UnknownSessionsJob,
    },
    key_version_negotiation_session::{
        LargestSupportResultComputer, SessionImpl as KeyVersionNegotiationSessionImpl,
        SessionParams as KeyVersionNegotiationSessionParams,
        SessionTransport as KeyVersionNegotiationTransport,
    },
    math,
    message::{
        ConfirmConsensusInitialization, ConfirmShareChangeSessionInitialization,
        ConsensusMessageWithServersSet, InitializeConsensusSessionWithServersSet,
        InitializeShareChangeSession, KeyVersionNegotiationMessage, Message,
        ServersSetChangeCompleted, ServersSetChangeConsensusMessage, ServersSetChangeDelegate,
        ServersSetChangeDelegateResponse, ServersSetChangeError, ServersSetChangeMessage,
        ServersSetChangeShareAddMessage, ShareChangeKeyVersionNegotiation, UnknownSessions,
        UnknownSessionsRequest,
    },
    share_change_session::{
        prepare_share_change_session_plan, ShareChangeSession, ShareChangeSessionParams,
        ShareChangeSessionPlan,
    },
    Error, KeyStorage, NodeId, SessionId,
};
...
type ServersSetChangeConsensusSession = ConsensusSession<
    ServersSetChangeAccessJob,
    ServersSetChangeConsensusTransport,
    UnknownSessionsJob,
...
struct SessionData {
    ...
    pub negotiation_sessions: BTreeMap<
        SessionId,
        KeyVersionNegotiationSessionImpl<ServersSetChangeKeyVersionNegotiationTransport>,
    >,
...
impl SessionImpl {
    ...
    pub fn initialize(
        ...
        let mut consensus_session = ConsensusSession::new(ConsensusSessionParams {
            meta: self
                ...
                .meta
                .clone()
                .into_consensus_meta(self.core.all_nodes_set.len())?,
            consensus_executor: ServersSetChangeAccessJob::new_on_master(
                self.core.admin_public.clone(),
    ...
    pub fn process_message(
        &self,
        sender: &NodeId,
        message: &ServersSetChangeMessage,
    ) -> Result<(), Error> {
    ...
    pub fn on_consensus_message(
        &self,
        sender: &NodeId,
        message: &ServersSetChangeConsensusMessage,
    ) -> Result<(), Error> {
                ...
                match &message.message {
                    &ConsensusMessageWithServersSet::InitializeConsensusSession(_) => {
                        data.consensus_session =
                            Some(ConsensusSession::new(ConsensusSessionParams {
                                meta: self
                                    .core
                                    .meta
                                    .clone()
                                    .into_consensus_meta(self.core.all_nodes_set.len())?,
                                consensus_executor: ServersSetChangeAccessJob::new_on_slave(
                                    self.core.admin_public.clone(),
                                ),
                                consensus_transport: ServersSetChangeConsensusTransport {
                                    id: self.core.meta.id.clone(),
        ...
        let unknown_sessions_job = UnknownSessionsJob::new_on_master(
            self.core.key_storage.clone(),
            self.core.meta.self_node_id.clone(),
        );
        consensus_session
            .disseminate_jobs(
                unknown_sessions_job,
                self.unknown_sessions_transport(),
                false,
            )
            .map(|_| ())
    }
    ...
    pub fn on_unknown_sessions_requested(
        &self,
        sender: &NodeId,
        message: &UnknownSessionsRequest,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        ...
        let new_nodes_set = {
            let consensus_session = data
                ...
                .as_mut()
                .ok_or(Error::InvalidMessage)?;
            let unknown_sessions_job =
                UnknownSessionsJob::new_on_slave(self.core.key_storage.clone());
            let unknown_sessions_transport = self.unknown_sessions_transport();

            // and respond with unknown sessions
            consensus_session.on_job_request(
                &sender,
                sender.clone(),
                unknown_sessions_job,
                unknown_sessions_transport,
            )?;
    ...
    pub fn on_unknown_sessions(
        &self,
        sender: &NodeId,
        message: &UnknownSessions,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        debug_assert!(sender != &self.core.meta.self_node_id);
        ...
        let unknown_sessions = {
            let consensus_session = data
                ...
                .as_mut()
                .ok_or(Error::InvalidMessage)?;
            consensus_session.on_job_response(
                sender,
                message
                    .unknown_sessions
                    .iter()
        ...
        data.state = SessionState::RunningShareChangeSessions;
        data.sessions_queue = Some(SessionsQueue::new(
            &self.core.key_storage,
            unknown_sessions.keys().cloned().collect(),
        ));
    ...
    pub fn on_key_version_negotiation(
        &self,
        sender: &NodeId,
        message: &ShareChangeKeyVersionNegotiation,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        debug_assert!(sender != &self.core.meta.self_node_id);
        ...
        match &message.message {
                ...
                if sender == &self.core.meta.master_node_id =>
            {
                ...
                let negotiation_session =
                    KeyVersionNegotiationSessionImpl::new(KeyVersionNegotiationSessionParams {
    ...
    pub fn on_initialize_share_change_session(
        &self,
        sender: &NodeId,
        message: &InitializeShareChangeSession,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        debug_assert!(sender != &self.core.meta.self_node_id);
        ...
        match data.active_key_sessions.contains_key(&key_id) {
            ...
            false => {
                let master_plan = ShareChangeSessionPlan {
                    ...
                    version_holders: message
                        .version_holders
                        .iter()
                        .cloned()
                        .map(Into::into)
                        .collect(),
                    consensus_group: message
                        .consensus_group
                        .iter()
                        .cloned()
                        .map(Into::into)
                        .collect(),
                    new_nodes_map: message
                        .new_nodes_map
                        .iter()
                        .map(|(k, v)| (k.clone().into(), v.clone().map(Into::into)))
                        .collect(),
                };
                ...
                if let Some(key_share) = self.core.key_storage.get(&key_id)? {
                    ...
                    if local_plan.new_nodes_map.keys().collect::<BTreeSet<_>>()
                        != master_plan.new_nodes_map.keys().collect::<BTreeSet<_>>()
                    {
                        return Err(Error::InvalidMessage);
                    }
                }

                let session = Self::create_share_change_session(
                    &self.core,
                    key_id,
                    master_node_id,
                    master_plan,
    ...
    pub fn on_share_change_session_confirmation(
        &self,
        sender: &NodeId,
        message: &ConfirmShareChangeSessionInitialization,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        debug_assert!(sender != &self.core.meta.self_node_id);
        ...
        let key_id = message.key_id.clone().into();
        let session_master = {
            let session_init_data = data
                .sessions_initialization_state
                .get_mut(&key_id)
                .ok_or(Error::InvalidMessage)?;
        ...
        if self.core.meta.self_node_id != session_master {
            data.delegated_key_sessions
                .insert(key_id, session_master.clone());
            return self.core.cluster.send(
                &session_master,
                Message::ServersSetChange(ServersSetChangeMessage::ServersSetChangeDelegate(
                    ServersSetChangeDelegate {
    ...
    pub fn on_sessions_delegation(
        &self,
        sender: &NodeId,
        message: &ServersSetChangeDelegate,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        debug_assert!(sender != &self.core.meta.self_node_id);
        ...
        let key_session = data
            .active_key_sessions
            .get_mut(&message.key_id.clone().into())
            .ok_or(Error::InvalidMessage)?;
        key_session.initialize()
    }

    /// When delegated session execution is completed.
    pub fn on_delegated_session_completed(
        &self,
        sender: &NodeId,
        message: &ServersSetChangeDelegateResponse,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        debug_assert!(sender != &self.core.meta.self_node_id);
    ...
    pub fn on_share_add_message(
        &self,
        sender: &NodeId,
        message: &ServersSetChangeShareAddMessage,
    ) -> Result<(), Error> {
    ...
    pub fn on_session_completed(
        &self,
        sender: &NodeId,
        message: &ServersSetChangeCompleted,
    ) -> Result<(), Error> {
        debug_assert!(self.core.meta.id == *message.session);
        debug_assert!(sender != &self.core.meta.self_node_id);
    ...
    fn on_share_change_message<F: Fn(&mut ShareChangeSession) -> Result<(), Error>>(
        &self,
    ...
    fn create_share_change_session(
        core: &SessionCore,
        key_id: SessionId,
        master_node_id: NodeId,
        session_plan: ShareChangeSessionPlan,
    ) -> Result<ShareChangeSession, Error> {
    ...
    fn disseminate_session_initialization_requests(
        core: &SessionCore,
        ...
        if data.sessions_queue.is_some() {
            ...
            let mut number_of_sessions_to_start =
                MAX_ACTIVE_KEY_SESSIONS.saturating_sub(number_of_sessions_active);
            while number_of_sessions_to_start > 0 {
                let key_id = match data
                    .sessions_queue
                    .as_mut()
                    .expect("checked before beginning of the loop; qed")
                    .next()
                {
                ...
                if !negotiation_session.is_finished() {
                    data.negotiation_sessions
                        .insert(key_id, negotiation_session);
                    continue;
                }
    ...
    /// Initialize share change session.
    fn initialize_share_change_session(
        core: &SessionCore,
        data: &mut SessionData,
        key_id: SessionId,
    ) -> Result<bool, Error> {
        ...
        let session_plan = prepare_share_change_session_plan(
            &core.all_nodes_set,
        ...
        let initialization_message = Message::ServersSetChange(
            ServersSetChangeMessage::InitializeShareChangeSession(InitializeShareChangeSession {
                ...
                consensus_group: session_plan
                    .consensus_group
                    .iter()
                    .cloned()
                    .map(Into::into)
                    .collect(),
                new_nodes_map: session_plan
                    .new_nodes_map
                    .iter()
        ...
        if need_create_session {
            data.active_key_sessions.insert(
                key_id.clone(),
                Self::create_share_change_session(
                    core,
                    key_id,
        ...
        if !wait_for_confirmations {
            ...
            data.sessions_initialization_state.insert(
                key_id,
                SessionInitializationData {
    ...
    fn return_delegated_session(core: &SessionCore, key_id: &SessionId) -> Result<(), Error> {
        ...
        core.cluster.send(
            ...
            Message::ServersSetChange(ServersSetChangeMessage::ServersSetChangeDelegateResponse(
                ServersSetChangeDelegateResponse {
                    session: core.meta.id.clone().into(),
                    session_nonce: core.nonce,
                    key_id: key_id.clone().into(),
                },
    ...
    fn complete_session(core: &SessionCore, data: &mut SessionData) -> Result<(), Error> {
        ...
        core.cluster.broadcast(Message::ServersSetChange(
            ServersSetChangeMessage::ServersSetChangeCompleted(ServersSetChangeCompleted {
                session: core.meta.id.clone().into(),
                session_nonce: core.nonce,
            }),
        ))?;
...
impl ClusterSession for SessionImpl {
    ...
    fn on_session_error(&self, node: &NodeId, error: Error) {
        ...
        if *node == self.core.meta.self_node_id {
            ...
            let _ = self.core.cluster.broadcast(Message::ServersSetChange(
                ServersSetChangeMessage::ServersSetChangeError(ServersSetChangeError {
                    session: self.core.meta.id.clone().into(),
                    session_nonce: self.core.nonce,
    ...
    fn send_partial_request(
        ...
        self.cluster.send(
            ...
            Message::ServersSetChange(ServersSetChangeMessage::ServersSetChangeConsensusMessage(
                ServersSetChangeConsensusMessage {
                    session: self.id.clone().into(),
                    session_nonce: self.nonce,
                    message: ConsensusMessageWithServersSet::InitializeConsensusSession(
                        InitializeConsensusSessionWithServersSet {
                            migration_id: self.migration_id.clone().map(Into::into),
                            old_nodes_set: request
                                .old_servers_set
                                .into_iter()
                                .map(Into::into)
                                .collect(),
                            new_nodes_set: request
                                .new_servers_set
                                .into_iter()
                                .map(Into::into)
                                .collect(),
        ...
        self.cluster.send(
            ...
            Message::ServersSetChange(ServersSetChangeMessage::ServersSetChangeConsensusMessage(
                ServersSetChangeConsensusMessage {
                    session: self.id.clone().into(),
                    session_nonce: self.nonce,
                    message: ConsensusMessageWithServersSet::ConfirmConsensusInitialization(
...
impl JobTransport for UnknownSessionsJobTransport {
    ...
    fn send_partial_request(&self, node: &NodeId, _request: NodeId) -> Result<(), Error> {
        self.cluster.send(
            node,
            Message::ServersSetChange(ServersSetChangeMessage::UnknownSessionsRequest(
                UnknownSessionsRequest {
                    session: self.id.clone().into(),
                    session_nonce: self.nonce,
                },
    ...
    fn send_partial_response(
        &self,
        node: &NodeId,
        response: BTreeSet<SessionId>,
    ) -> Result<(), Error> {
        self.cluster.send(
            node,
            Message::ServersSetChange(ServersSetChangeMessage::UnknownSessions(UnknownSessions {
                session: self.id.clone().into(),
                session_nonce: self.nonce,
...
impl KeyVersionNegotiationTransport for ServersSetChangeKeyVersionNegotiationTransport {
    fn broadcast(&self, message: KeyVersionNegotiationMessage) -> Result<(), Error> {
        self.cluster.broadcast(Message::ServersSetChange(
            ServersSetChangeMessage::ShareChangeKeyVersionNegotiation(
                ShareChangeKeyVersionNegotiation {
                    session: self.id.clone().into(),
                    session_nonce: self.nonce,
                    message: message,
                },
...
pub mod tests {
    ...
    impl<S: ClusterSession> MessageLoop<S> {
        ...
        pub fn with_ml<C: AdminSessionAdapter<S>>(
            ...
            let mut all_nodes_set: BTreeSet<_> = ml
                .nodes()
                .into_iter()
            ...
            let new_nodes_set: BTreeSet<NodeId> = all_nodes_set
                .iter()
                .cloned()
            ...
            let sessions: BTreeMap<_, _> = (0..ml.nodes().len())
                .map(|idx| {
                    (
                        ml.node(idx),
                        C::create(meta.clone(), admin_public, all_nodes_set.clone(), &ml, idx),
                    )
                })
                .collect();

            let all_set_signature = sign(
                admin_key_pair.secret(),
                &ordered_nodes_hash(&old_set_to_sign),
            )
            .unwrap();
    ...
    pub fn generate_key(num_nodes: usize, threshold: usize) -> GenerationMessageLoop {
        let gml = GenerationMessageLoop::new(num_nodes)
            .init(threshold)
            .unwrap();
    ...
    fn node_added_using_servers_set_change() {
        ...
        let ml =
            MessageLoop::with_gml::<Adapter>(gml, master, Some(add), None, None).run_at(master);
    ...
    fn node_moved_using_servers_set_change() {
        ...
        let add = vec![Random.generate().unwrap()];
        let ml =
            MessageLoop::with_gml::<Adapter>(gml, master, Some(add), Some(remove.clone()), None)
                .run_at(master);

        // check that secret is still the same as before moving the share
        ml.check_secret_is_preserved(ml.sessions.keys().filter(|k| !remove.contains(k)));

        // check that all removed nodes do not own key share
        assert!(ml.sessions.keys().filter(|k| remove.contains(k)).all(|k| ml
            .ml
            .key_storage_of(k)
    ...
    fn node_removed_using_servers_set_change() {
        ...
        let master = gml.0.node(0);
        let ml = MessageLoop::with_gml::<Adapter>(gml, master, None, Some(remove.clone()), None)
            .run_at(master);
    ...
    fn isolated_node_removed_using_servers_set_change() {
        ...
        // try to recover secret for every possible combination of nodes && check that secret is the same
        ml.check_secret_is_preserved(ml.sessions.keys().filter(|k| !isolate.contains(k)));
    ...
    fn having_less_than_required_nodes_after_change_does_not_fail_change_session() {
        ...
        let master = add[0].public().clone();
        let ml = ml
            .and_then::<Adapter>(master, Some(add.clone()), None, Some(remove))
            .run_at(master);
        ...
        assert!(ml
            .sessions
            .keys()
            .filter(|k| add.iter().any(|n| n.public() == *k))
            .all(|k| ml
                .ml
                .key_storage_of(k)
                .get(&SessionId::default())
    ...
    fn adding_node_that_has_lost_its_database_works() {
        ...
        let ml = ml
            .and_then::<Adapter>(master, Some(add), None, None)
            .run_at(master);
