...
use key_server_cluster::{message::Message, Error, NodeId};
...
#[cfg(test)]
pub mod tests {
    use super::{Connection, ConnectionManager, ConnectionProvider};
    use key_server_cluster::{message::Message, Error, NodeId};
    ...
    impl TestConnections {
        pub fn isolate(&self) {
            let connected_nodes =
                ::std::mem::replace(&mut *self.connected_nodes.lock(), Default::default());
    ...
    impl Connection for TestConnection {
        ...
        fn send_message(&self, message: Message) {
            self.messages
                .lock()
                .push_back((self.from, self.to, message))
        }
    }
